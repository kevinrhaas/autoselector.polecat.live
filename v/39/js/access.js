...
// The admin token itself is NEVER stored in this repo or shown on the public
// marketing site — it lives only on the admin's device (and wherever the
// admin chooses to keep it).
// -----------------------------------------------------------------------

// Public key that matches the admin token. Anyone can verify with it.
const PUBLIC_KEY_B64 =
  'MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEBboIeLl4enQt1VpTnAUHa/oGUFYicRGEpKC0gMzek5rqDfKdisfJSw0/6ci1CG627aKmsUVQoyOxLZjuUtT8bQ==';

const A_KEY   = 'as.access';    // { grantedAt, via, label }
const ADM_KEY = 'as.adminkey';  // admin private key (pkcs8 b64) — admin device only
const INV_KEY = 'as.invites';   // locally-kept list of minted invites
const REV_KEY = 'as.revoked';   // locally-revoked jti list
...
export const Access = new (class {
  ...
  async verifyAdminToken(token){
    try{
      ...
      const msg = enc.encode('autoselector-admin-check');
  ...
  async mintInvite({ label='', days=0, vehicle='' }={}){
    ...
    // A share link may deep-link to a specific vehicle.
    const hash = vehicle ? `#vehicle/${encodeURIComponent(vehicle)}` : '';
    const link = `${location.origin}/app/?token=${encodeURIComponent(code)}${hash}`;
    this._remember({ label, iat, exp, jti, code, link, vehicle });
