// check-changelog.mjs — guard that js/changelog.js and js/whatsnext.js stay
// parseable by the rest of the fleet (manager's "Sync changelog").
//
// It re-implements manager's EXACT parser (ingest.js: extractArrayLiteral +
// jsLiteralToJSON) and runs it against our published data files. If manager
// couldn't parse them, this fails — so the smoke test catches it before deploy
// instead of the owner finding out in manager.
//
...
let failed = false;
for(const [file, varName] of [['js/changelog.js','CHANGELOG'], ['js/whatsnext.js','WHATSNEXT']]){
