# autoselector.polecat.live — agent guide

AutoSelector, the 2026-model-year car finder. Local-first, vanilla HTML/JS/CSS
single-page app (no framework, no build step, ES modules) on GitHub Pages,
served under `/app/`. Read `.github/self-improve-prompt.md` for the full
working ground rules and `ROADMAP.md` for direction.
...
`theme.js` / `icons.js` / `shell.js` from the vendor path; the app-local
`js/icons.js` layers AutoSelector's automotive icon family (car/suv/truck/
van/…) on top via `registerIcons()`, and theming keeps the historical key
(`as.theme.v1`) via `configure()`.
...
- **Local-first**: data in `localStorage` (`as.workspace`) via `js/store.js`
